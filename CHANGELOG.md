...
- Auto-demotion bench (`tests/bench/demotion.py`) now captures tick p50/p99 sampled from the new INFO fields, AIMD stall events per cell, and NVMe write amplification (bytes written per committed demotion); value-size matrix extended to 64 KiB; report + CSV written to `tests/bench/results/demotion_v1.1.0.{md,csv}` to match the spec filename for the adaptive-batch task's threshold tuner.
